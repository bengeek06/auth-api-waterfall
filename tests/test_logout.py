"""
Tests for the logout resource.

This module contains tests for the LogoutResource API endpoint and token revocation.
"""
...
def test_logout_success(client, session):
    ...
    rt = RefreshToken(token=refresh_token, user_id=1, company_id=42, expires_at=expires)
    db.session.add(rt)
    db.session.commit()
