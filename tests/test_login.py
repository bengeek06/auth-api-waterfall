"""
Tests for the login resource.

This module contains tests for the LoginResource API endpoint and authentication.
"""
...
def test_login_success(client, monkeypatch):
    ...
    def mock_check_credentials_success(_, __):
        return user

    monkeypatch.setattr("app.resources.login.check_credentials", mock_check_credentials_success)
...
def test_login_invalid_credentials(client, monkeypatch):
    ...
    def mock_check_credentials_failure(_, __):
        return None

    monkeypatch.setattr("app.resources.login.check_credentials", mock_check_credentials_failure)
    response = client.post(
        "/login", json={"email": "wrong@example.com", "password": "badpass"}
    )
